...
例如  
```10000000 00000000 00000000 00000001```是負的。  
```00000000 00000000 00000000 00000001```是正的。  
...
= (-1) * (1111111 11111111 11111111 11111110 + 1)  
= (-1) * ((2^1 + 2^2 + 2^3 + ... + 2^29 + 2^30) + 1) = -2147483647
```
...
*    int最小負整數  
```10000000 00000000 00000000 00000000 = -(2147483647+1) = -2147483648```  
...
往左移4個位置  
```00000000 00000000 00000000 01100000 = 96 = 6 * (2^4)```  
...
往右移4個位置  
```00000000 00000000 00000000 00000110 = 6 = 96 / (2^4)```  
...
再向右移4個位置  
```00001000 00000000 00000000 00000000 = 134217728```  
答案可是不會變回來的喔
...
往右移1個位置，右邊再補滿原本的符號，即1(負號)  
```11111111 11111111 11111111 11111111 = -1 ＝ -2 / 2```  
原本是負的右移之後還是負的喔  
聰明如你，原本是正的右移之後也會是正的。  
...
首先```((1L << 48) - 1)```為2的48次方減1，因為把二進位的1向左移48個位置再補0，轉成十進位，再減1。  
